...
ruleset = tuple(map(lambda l: tuple(map(int, l.split(' '))), real.split('\n')))
...
def run2(config_file):
    ...
    winner = p.run(lambda a,b:eval_genomes(a,b,ruleset[0]), 501)
    ...
    #print('\nOutput:')
    winner_net = neat.nn.FeedForwardNetwork.create(winner, config)
    print(play_game(winner_net, ruleset[0], True))
    #for xi, xo in zip(xor_inputs, xor_outputs):
    #    output = winner_net.activate(xi)
    #    print("input {!r}, expected output {!r}, got {!r}".format(xi, xo, output))

    # node_name_map = ["turn", "P1 actions", "P2 actions"]
    # node_name_map_entity = ["team", "hp", "hpMax", "mp", "mpMax", "A1 cost", "A1 dmg", "A2 cost", "A2 dmg"]
    # for i in range(1, 4+1):
    #     for n in node_name_map_entity:
    #         node_name_map.append(f"#{i} {n}")
    # node_names = {-1: 'turn', -2: 'P1 remaining', -3: '', 0: 'eval'}
    # for i, name in enumerate(node_name_map):
    #     node_names[-i] = name
    # print(name)
